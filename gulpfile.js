...
gulp.task('music', () => {
  return gulp.src(config.dev.music)
    .pipe(gulp.dest(config.build.music))
})

gulp.task('plugin', () => {
  return gulp.src(config.dev.plugin)
    .pipe(gulp.dest(config.build.plugin))
})
...
gulp.task('server', () => {
  const task = ['html', 'styles', 'script', 'images', 'static', 'music', 'plugin']
  cbTask(task).then(() => {
